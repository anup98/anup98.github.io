...
function touchmove(ev) {
  ev.preventDefault();
  if (!touch) return; //Draw path for each touch
...
window.onload = function () {
  ...
  A_DATA = new CompoundPath({
    children: [new Path({
      ...
      segments: [[35, 0], [70, 100]]
    }), new Path({
      segments: [[17.5, 50], [52.5, 50]]
    })]
  });
  D_DATA = new CompoundPath({
    children: [new Path({
      ...
      segments: [[[0, 0], null, [75, 0]], [[0, 100], [75, 0], null]]
    })]
  });
  ...
  K_DATA = new CompoundPath({
    children: [new Path({
      ...
      segments: [[50, 0], [0, 50], [50, 100]]
    })]
  });
  Q_DATA = new CompoundPath({
    children: [new Path({
      segments: [[[50, 0], [30, 0], [-30, 0]], [[0, 50], [0, -30], [0, 30]], [[50, 100], [-30, 0], [30, 0]], [[100, 50], [0, 30], [0, -30]], [[50, 0], [30, 0], [-30, 0]]]
    }), new Path({
      segments: [[75, 75], [100, 100]]
    })]
  });
  S_DATA = new CompoundPath({
    // TODO: Fix this 'S'
    children: [new Path({
      segments: [[[40, 15], null, [-35, -15]], [[25, 47], [-30, -10], [35, 5]], [[0, 80], [50, 25], null]]
    })]
  });
...
function () {
   ...
   * @memberof Letter
   */
  function Letter(paths, progression) {
  ...
  _createClass(Letter, [{
    key: "checkBounds",
    value: function checkBounds(point) {
